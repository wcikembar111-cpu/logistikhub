...
export function LinkGrid({ links, loading, isAdmin, onAdd, onEdit, onDelete }: LinkGridProps) {
  ...
  return (
    <>
      <div className="glass-box flex items-center mb-6 p-2 !rounded-full bg-white/40 border border-white/60">
      ...
      <div className="flex justify-center gap-3 mb-8 flex-wrap">
        ...
        {isAdmin && (
          <button onClick={onAdd} className="glass-btn !bg-orange-500/80 !text-white hover:!bg-orange-500 !border-orange-400 !px-5 !rounded-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-4">
        {loading ? (
          ...
          filteredLinks.map((l, index) => {
            ...
            return (
              <div key={l.id} className="glass-box p-6 flex flex-col relative hover:-translate-y-2 hover:shadow-2xl transition-all cursor-pointer group bg-white/30 hover:bg-white/50">
                <div className="flex justify-between items-start mb-5">
                  <div className={`w-16 h-16 rounded-2xl flex items-center justify-center text-4xl shrink-0 shadow-sm backdrop-blur-md border border-white/60 ${iconBg}`}>
                    {isEmoji ? <span className="drop-shadow-sm">{l.icon}</span> : <i className={l.icon || 'fas fa-link'} />}
                  </div>
                  ...
                  {isAdmin && (
                    <div className="hidden group-hover:flex gap-2 z-10">
                      ...
                      <button 
                        ...
                        className="glass-btn !p-3 !rounded-xl !bg-red-500/10 hover:!bg-red-500/20 !text-red-600"
                      >
                        <Trash2 size={18} />
                      </button>
                    </div>
                ...
                <div className="flex-1 flex flex-col min-w-0">
                  <h4 className="font-extrabold text-xl text-slate-800 m-0 mb-1 truncate uppercase">
                    {l.title}
                  </h4>
                  <p className="text-xs font-semibold text-slate-500 m-0 mb-4 truncate uppercase">
                    {l.category}
                  </p>
                  
                  <div className="mt-auto pt-2">
                    {l.subcategory && (
                      <span className="px-3 py-1.5 bg-white/60 border border-white/80 text-orange-600 rounded-lg text-[10px] font-bold uppercase tracking-wider shadow-sm">
                        {l.subcategory}
                      </span>
                    )}
                  </div>
